...
urlpatterns = [
    ...
    re_path(r'^questions/(?P<pk>-?[0-9]\d*)/$', QuestiondetailsView.as_view({'get':'retrieve'})),#7问题详情
    re_path(r'^questions/(?P<pk>-?[0-9]\d*)/useful/$',UsefulQuestionView.as_view()),#8问题有用
    re_path(r'^questions/(?P<pk>-?[0-9]\d*)/unuseful/$',UnusefulQuestionView.as_view()),#9.问题没用
    re_path(r'^reply/$',ReplyView.as_view()),
    re_path(r'^labels/(?P<pk>-?[0-9]\d*)/focusin/$', AttentionstagsView.as_view()),#关注标签
    re_path(r'^labels/(?P<pk>-?[0-9]\d*)/focusout/$', AttentiontagsView.as_view()),#关注标签
    re_path(r'^labels/(?P<pk>-?[1-9]\d*)/$', LabelView.as_view({'get':'retrieve'})),#标签详情
